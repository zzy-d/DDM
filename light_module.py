...
class LTModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #print('auroc:', (auc / logits.shape[0]).item())
    ...
    def test_epoch_end(self, outputs):
        ...
        ndcg5 = []
        for label, pred in zip(group_labels, group_preds,):
             ndcg5.append(ndcg_score(label, pred, 5))
        ndcg5 =np.mean(ndcg5)
        print("ndcg5:", ndcg5)

        ndcg10 = []
        for label, pred in zip(group_labels, group_preds, ):
            ndcg10.append(ndcg_score(label, pred, 10))
        ndcg10 = np.mean(ndcg10)
        print("ndcg10:", ndcg10)

        # auc = []
        # for label, pred in zip(group_labels, group_preds):
        #    auc.append(auroc(np.array(pred), np.array(label)))
        # auc = np.mean(auc)
        # print(auc)
        ...
        auc = []
        for label, pred in zip(group_labels, group_preds):
            label = torch.tensor(label, dtype=torch.long) # 将标签类型转换为整数类型
            pred = torch.tensor(pred, dtype=torch.float32)  # 将列表转换为 PyTorch 张量
            auc.append(auroc(pred, label))  # 使用 Tropchmetric 函数计算 AUC
        auc = torch.mean(torch.stack(auc))
        print("auc:", auc)
